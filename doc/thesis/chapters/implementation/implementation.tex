...
\begin{itemize}
    \item \textit{record timestamp}: elapsed time (in seconds) since the start of the session as recorded by the network monitoring device used in the creation of the Balabit dataset
    \item \textit{client timestamp}: elapsed time (in seconds) since the start of the session as recorded by the RDP client used by each of the 10 users
    \item \textit{button}: the current condition of the mouse buttons
    \item \textit{state}: additional information about the current state of the mouse
    \item \textit{x}: the x coordinate (in pixels) of the mouse cursor on the screen
    \item \textit{y}: the y coordinate (in pixels) of the mouse cursor on the screen
\end{itemize}
